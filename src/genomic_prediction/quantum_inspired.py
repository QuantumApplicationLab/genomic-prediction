...
def sample_C(A, m, n, r, c, row_norms, LS_prob_rows, LS_prob_columns, A_Frobenius):
    ...
    for s in range(r):
        R_row[:] = (
            A[rows[s], :] * A_Frobenius / (np.sqrt(r) * np.sqrt(row_norms[rows[s]]))
        )
    ...
    for i in range(reps):
        ...
        for l in range(rank):
            ...
            for k in range(samples):
                ...
                for s in range(r):
                    v_j += (
                        A[rows[s], sample_j] * w[s, l] / (np.sqrt(row_norms[rows[s]]))
                    )
                    # print(v_j)
...
def sample_from_x(
    A, r, n, rows, row_norms, LS_prob_columns_R, A_Frobenius, w_vector, w_norm
):
...
def approx_solution_rsys(
    A, rank, r, w, rows, sigma, row_norms, A_Frobenius, lambdas, comp
):
...
def linear_eqs(A, b, r, c, rank, Nsamples, NcompX):
    ...
    tic = time.time()
    lambdas = sample_me_lsyst(
        A, b, m_rows, n_cols, Nsamples, rank, r, *svd_C[0:3], *LS[0:4]
    )
    toc = time.time()
    ...
    for t in range(NcompX):
        sampled_comp[t], n_of_rejected_samples[t] = sample_from_x(
            A, r, n_cols, svd_C[1], LS[0], svd_C[4], LS[3], w_vector, w_norm
        )
    ...
    for t in range(NcompX):
        x_tilde[t] = approx_solution(
            A, rank, r, w, svd_C[1], svd_C[2], LS[0], LS[3], lambdas, sampled_comp[t]
        )
...
def recomm_syst(A, user, r, c, rank, Nsamples, NcompX):
    ...
    lambdas = sample_me_rsys(
        A, user, n_cols, Nsamples, rank, r, *svd_C[0:3], LS[0], *LS[2:4]
    )
    ...
    for t in range(NcompX):
        x_tilde[t] = approx_solution_rsys(
            A, rank, r, w, svd_C[1], svd_C[2], LS[0], LS[3], lambdas, sampled_comp[t]
        )
...
def linear_eqs_portopt(A, mu, r, c, rank, Nsamples, NcompX):
    ...
    tic = time.time()
    lambdas = sample_me_rsys(
        A, 0, n_cols, Nsamples, rank, r, *svd_C[0:3], LS[0], *LS[2:4]
    )
    lambdas = mu * lambdas
    toc = time.time()
    rt_sampling_me = toc - tic
